...
| `engram instructions apply <id>` | Locally apply one approved `project_instruction` revision with fail-closed Git/instruction-graph/marker/encoding/CAS checks, atomic permission/newline-preserving replacement, recovery backup, and typed success or failure audit. |
...
SHA-256 identity for its staging checkout plus canonical target, which apply must reproduce before a
...
write completed but reporting did not, a proposal-bound HMAC receipt in local
...
routing and approved-rules domains, verifies the staging checkout identity and
clean target/Git operation state, writes the single canonical file atomically
with a recovery backup, and records the proposer, approver, applier, hashes,
and outcome. Malformed markers, ambiguous anchors, changed/dirty targets,
unsafe paths or symlinks, unresolved/cyclic/external imports, unsupported
encodings, and write races fail closed before overwrite. The single writer
records one typed `conflict`/`failed` event with repair guidance and never a
false `applied` event; no force/merge/rebase or staging fallback exists. Safe
adapters, unowned bytes, permissions, and newline style are preserved. The
server/MCP never writes the repository. Move operations remain review-only in
this first single-target executor.
