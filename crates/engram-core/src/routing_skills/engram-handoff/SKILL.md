...
- `memory_handoff_begin` creates a WorkItem and open Handoff at session end, or publishes another Handoff for an exact existing WorkItem owned by the same authenticated actor and source Run. Attach a bounded brief and revisioned ContextRefs; do not copy canonical page bodies into the Handoff. Attach typed ArtifactRefs (`file`, `git`, `worktree`, `external`) and explicit `depends_on` / `derived_from` / `child_of` relationships on this existing tool; do not invent a new MCP tool for artifacts. Absolute cwd is a local-path hint, never identity.
