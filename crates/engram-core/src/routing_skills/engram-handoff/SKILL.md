...
Recovery has two entry points and one shared contract. An Agent Adapter whose harness reliably delivers session-start output receives the continuation **already claimed for this Run**, rendered with the same envelope, claim metadata, ContextPackage, provenance, and assembly semantics `memory_handoff_claim` returns. Use that block directly. Do not call `memory_handoff_claim` for it, and do not re-explore what it already states. Acknowledge it with your first `memory_checkpoint_write` for that WorkItem, passing the `handoff_id`, `claim_id`, `expected_handoff_revision`, `expected_work_item_revision`, and `run_id` the block names. Call `memory_handoff_release` if you will not continue; simply stopping also works, because the lease expires and the transfer returns to `open`.

A block headed "pending handoff" rather than "continuation CLAIMED" means nothing was claimed — that adapter could not report its Run — so claim it explicitly before continuing.

A harness that ignores session-start output receives no block and engram performs no automatic read or mutation for it. Those agents recover on demand: `memory_handoff_discover`, then `memory_handoff_claim`.

Rendering or injecting a continuation never accepts it. Only the receiving Run's first valid Checkpoint does. There is no accept operation; do not look for one.
