...
printf '%s' "$PAYLOAD" \
    | engram_post_hook "$SERVER/hook?event=session-start&agent=cursor${QS}" >/dev/null 2>&1 || true
engram_get_handoff "$SERVER/handoff?agent=cursor${QS}${RUN_QS}" 2>/dev/null || true
exit 0
